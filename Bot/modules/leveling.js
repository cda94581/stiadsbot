...
module.exports = message => {
	...
	con.connect(err => { // Connect to the database
		...
		sql = 'CREATE DATABASE IF NOT EXISTS stiadsbot_leveling'; // Create a database called stiadsbot_leveling if it doesn't already exist
				...
				con.query(sql, err => {
					...
					con.query(sql, (err, rows) => {
						...
						if (!rows.length) { // If it doesn't return any results
							sql = `INSERT INTO stiads_xp (id, xp, level) VALUES (${author}, 0, 0)` // Insert a row with the id of the message author, and xp and level as 0
							con.query(sql, err => {
								if (err) throw err;
							});
						}
						...
						sql = `UPDATE stiads_xp SET xp = xp + ${addXp} WHERE id = ${author}`; // Update user xp to add 15-25
						con.query(sql, (err, rows) => {
							if (err) throw err;
							sql = `SELECT * FROM stiads_xp WHERE id = ${author}`;
							con.query(sql, (err, rows) => {
								if (err) throw err;
								xp = rows[0].xp;
								level = rows[0].level;
	...
	function leveling() {
		...
		if (xpToLevel < xp) {
			...
			con.query(sql, err => {
				...
				sql = `UPDATE stiads_xp SET level = ${level} WHERE id = ${author}`; // Increase in database
				con.query(sql, err => {
					if (err) throw err;
