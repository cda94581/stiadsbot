...
module.exports = {
	...
	execute(message, args) {
		if (!args.length) return message.channel.send({ embeds: [ new Discord.MessageEmbed().setColor('#444444').setTitle('FAQ').setDescription('These are some handy things to know! Use the `faq help` command to get a list of all queries. Use `faq [faq name]` (without brackets) to use the command') ]});
