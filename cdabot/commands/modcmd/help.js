...
module.exports = {
	...
	execute(message, args) {
		...
		if (args.length) { // If it asks for a specific modcmd help
			...
			if (!modcmd) return message.channel.send('That\'s not a valid Mod Command');
			data[0] = '';
			...
			return message.channel.send(new Discord.MessageEmbed().setColor('#44434d').setTitle(modcmd.name).setDescription(data[0]));
		}
		...
		for (i = 0; i < data.length; i++) {
			...
			message.channel.send(new Discord.MessageEmbed().setColor('#444444').setTitle('CdaBot Mod Command Help').setDescription(data[i]));
		}
	}
}