...
module.exports = {
	...
	execute(message, args) {
		...
		fs.readFile(filePath, 'utf-8', (err, data) => {
			...
			message.channel.send({ embeds: [ new Discord.MessageEmbed().setColor('#ff0000').setTitle(`STIADS:eyes: Bot Leveling - ${message.guild.members.cache.find(m => m.id == author).displayName}`).setDescription(`**Level**: ${info.level}\n**XP**: ${info.xp}/${toLevelUp}`) ]});
		});
	},
};