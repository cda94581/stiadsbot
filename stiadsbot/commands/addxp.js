...
module.exports = {
	name: 'addxp',
	description: 'Adds XP for a user, you will currently have to manually set the roles',
	type: 'leveling',
	args: true,
	perms: [ 'ADMINISTRATOR' ],
	usage: '<USER> <LEVEL> <XP>',
	execute(message, args, client) {
		...
		con.connect(err => {
			...
			con.query(sql, (err, rows) => {
				if (err) throw err;
				message.channel.send(`**${user.username}**'s level has been increased by ${args[1]} and xp has increased by ${args[2]}`);
				con.end();
			});
		});
	},
