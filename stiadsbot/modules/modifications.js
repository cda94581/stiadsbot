...
module.exports = (message, client) => {
	...
	const embed = new Discord.MessageEmbed().setColor('#ff0000').setTitle(`Modification Submission - ${message.author.username}#${message.author.discriminator}`).setDescription(message.content).attachFiles(attachments);
	client.channels.cache.find(channel => channel.id == modifications.channel).send(embed).then(sentMsg => sentMsg.react('👍'));
	try {
